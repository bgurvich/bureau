<form wire:submit="save" class="space-y-4" novalidate>
    ...
    @php($pet = $pv_pet_id ? \App\Models\Pet::find($pv_pet_id) : null)
    ...
    @error('pv_pet_id')<div role="alert" class="text-xs text-rose-400">{{ $message }}</div>@enderror
    ...
    <div>
        ...
        <input wire:model="pv_vaccine_name" id="i-pv-name" type="text" required autofocus
               list="i-pv-name-opts"
        ...
        @endif
        @error('pv_vaccine_name')<div role="alert" class="mt-1 text-xs text-rose-400">{{ $message }}</div>@enderror
    </div>

    <div class="grid grid-cols-2 gap-3">
        <div>
            <label for="i-pv-admin" class="mb-1 block text-xs text-neutral-400">{{ __('Administered on') }}</label>
            <input wire:model="pv_administered_on" id="i-pv-admin" type="date"
                   class="w-full rounded-md border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-100 focus-visible:border-neutral-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-neutral-300">
        </div>
        <div>
            <label for="i-pv-valid" class="mb-1 block text-xs text-neutral-400">{{ __('Valid until') }}</label>
            <input wire:model="pv_valid_until" id="i-pv-valid" type="date"
                   class="w-full rounded-md border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-100 focus-visible:border-neutral-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-neutral-300">
            @error('pv_valid_until')<div role="alert" class="mt-1 text-xs text-rose-400">{{ $message }}</div>@enderror
        </div>
    </div>

    <div>
        <label for="i-pv-booster" class="mb-1 block text-xs text-neutral-400">{{ __('Booster due on') }}</label>
        <input wire:model="pv_booster_due_on" id="i-pv-booster" type="date"
               class="w-full rounded-md border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-100 focus-visible:border-neutral-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-neutral-300">
    ...
    <div>
        ...
        <textarea wire:model="pv_notes" id="i-pv-notes" rows="2"
