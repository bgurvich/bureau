<form wire:submit="save" class="space-y-4" novalidate>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div>
            ...
            <input wire:model="te_activity_date" id="i-te-date" type="date" required autofocus
            ...
            @error('te_activity_date')<div role="alert" class="mt-1 text-xs text-rose-400">{{ $message }}</div>@enderror
        </div>
        <div>
            <label for="i-te-hours" class="mb-1 block text-xs text-neutral-400">{{ __('Hours') }}</label>
            <input wire:model="te_hours" id="i-te-hours" type="number" step="0.25" min="0.01" max="24" required
                   placeholder="2.5"
            ...
            @error('te_hours')<div role="alert" class="mt-1 text-xs text-rose-400">{{ $message }}</div>@enderror
        </div>
    </div>
    ...
    <div>
        ...
        <x-ui.searchable-select
            ...
            placeholder="—" />
        @error('te_project_id')<div role="alert" class="mt-1 text-xs text-rose-400">{{ $message }}</div>@enderror
    </div>
    ...
    <div>
        ...
        <x-ui.searchable-select
            ...
            placeholder="—" />
        @error('te_task_id')<div role="alert" class="mt-1 text-xs text-rose-400">{{ $message }}</div>@enderror
    </div>

    <div>
        <label for="i-te-desc" class="mb-1 block text-xs text-neutral-400">{{ __('Description') }}</label>
        <textarea wire:model="te_description" id="i-te-desc" rows="2"
        ...
        @error('te_description')<div role="alert" class="mt-1 text-xs text-rose-400">{{ $message }}</div>@enderror
    </div>

    <label class="flex items-center gap-2 text-sm text-neutral-300">
        <input wire:model="te_billable" type="checkbox" class="rounded border-neutral-700 bg-neutral-950">
        {{ __('Billable') }}
    </label>
</form>
