...
class extends Component
{
    ...
    {
        $this->tab = HubTabMemory::resolve('assets', $this->tab, 'properties');
    }

    public function setTab(string $tab): void
    {
        if (in_array($tab, ['properties', 'vehicles', 'vehicle_services', 'inventory', 'locations', 'listings', 'domains', 'meters', 'online_accounts', 'in_case_of'], true)) {
            $this->tab = $tab;
    ...
    <div role="tabpanel"
        ...
        @switch($tab)
            ...
            @case('inventory')
                ...
                @break
            @case('locations')
                <livewire:locations-index :key="'assets-locations'" />
                @break
            @case('listings')
                <livewire:listings-index :key="'assets-listings'" />
                @break
            @case('domains')
                <livewire:domains-index :key="'assets-domains'" />
                @break
            @case('meters')
                <livewire:meter-readings-index :key="'assets-meters'" />
                @break
