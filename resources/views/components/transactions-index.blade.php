...
class extends Component
{
    ...
    public function transactions()
    {
        return Transaction::query()
            ->with(['account:id,name,currency', 'category:id,name,slug', 'counterparty:id,display_name', 'tags:id,name,slug'])
...
<div class="space-y-5">
    ...
    <dl class="flex gap-6 text-right">
            <div>
                ...
                <dd class="mt-0.5 text-sm tabular-nums text-emerald-400">+{{ number_format($this->totals['credits'], 2) }}</dd>
            </div>
            <div>
                <dt class="text-[10px] uppercase tracking-wider text-neutral-500">{{ __('Out') }}</dt>
                <dd class="mt-0.5 text-sm tabular-nums text-rose-400">{{ number_format($this->totals['debits'], 2) }}</dd>
            </div>
            <div>
                ...
                <dd class="mt-0.5 text-sm tabular-nums {{ $this->totals['net'] >= 0 ? 'text-emerald-400' : 'text-rose-400' }}">
                    {{ ($this->totals['net'] >= 0 ? '+' : '').number_format($this->totals['net'], 2) }}
                </dd>
            </div>
    </dl>
                    ...
                    @foreach ($this->transactions as $t)
                        <tr wire:click="$dispatch('inspector-open', { type: 'transaction', id: {{ $t->id }} })"
                            ...
                            <td class="px-3 py-2 text-right tabular-nums {{ $t->amount >= 0 ? 'text-emerald-400' : 'text-rose-400' }}">
                                {{ ($t->amount >= 0 ? '+' : '').number_format((float) $t->amount, 2) }}
                            </td>
