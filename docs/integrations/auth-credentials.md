...
How to provision the keys, secrets, and OAuth clients Secretaire's integrations expect. Every entry here is for the **production** tenant — use a separate OAuth client in dev with `http://localhost` redirects, or the provider's sandbox mode, so rotation is safe.

## Credential storage model

Two layers:

1. **`.env`** — process-scoped secrets Laravel needs to boot (`GOOGLE_CLIENT_ID`, `POSTMARK_WEBHOOK_PASSWORD`, `AWS_SECRET_ACCESS_KEY`). Mode `640`, owner `secretaire:www-data`. Never committed. Rotation = edit `.env` + `php artisan config:clear`.
...
3. Authorised redirect URI: `https://secretaire.aurnata.com/auth/google/callback`.
4. Copy **Client ID** + **Client secret**:

```env
GOOGLE_CLIENT_ID=…
GOOGLE_CLIENT_SECRET=…
# GOOGLE_REDIRECT_URL=…  # override if Secretaire runs on a non-canonical URL
```
...
2. Homepage URL: `https://secretaire.aurnata.com`. Authorization callback URL: `https://secretaire.aurnata.com/auth/github/callback`.
...
3. Redirect URI: **Web** → `https://secretaire.aurnata.com/auth/microsoft/callback`.
...
2. **Identifiers → + → Services IDs**. Description: *Secretaire*. Identifier: e.g. `homes.secretaire.signin` (NOT `com.apple.…`). Save.
3. Re-open the Services ID → enable **Sign in with Apple** → **Configure** → Primary App ID: a registered App ID (create one if needed) → Domains and Return URLs: `secretaire.aurnata.com` + `https://secretaire.aurnata.com/auth/apple/callback`.
...
php artisan tinker --execute='
    ...
    $teamId = "YOUR_TEAM_ID";
    $kid    = "YOUR_KEY_ID";
    $sub    = "homes.secretaire.signin";   // the Services ID
...
```env
APPLE_CLIENT_ID=homes.secretaire.signin
...
4. User visits `/integrations/gmail/connect` → redirects to Google → consents → Secretaire stores the refresh token.
...
2. Scope: `Mail (read)`. Name: `secretaire`. Copy the generated token.
...
3. Under the app → **Webhooks → Add Webhook** → URL `https://secretaire.aurnata.com/webhooks/paypal` → subscribe to the invoice/payment events Secretaire consumes.
4. Connect:
...
Postmark POSTs each received email to an endpoint on Secretaire. Auth is HTTP Basic, embedded in the URL Postmark is configured with. `App\Http\Controllers\PostmarkInboundController` fails closed in production when credentials are blank (covered by `WebhookAuthTest`).

1. Postmark → **Servers → <your server> → Inbound settings**.
2. Generate a random `user:pass` — e.g. `openssl rand -hex 16` for each half.
3. Webhook URL: `https://user:pass@secretaire.aurnata.com/webhooks/postmark/inbound`.
...
```json
{
    "Statement": [{
        ...
        "Resource": ["arn:aws:s3:::secretaire-backups", "arn:aws:s3:::secretaire-backups/*"]
    }]
}
```
...
- **OAuth refresh tokens** can be invalidated by the provider at any time (user revoke, password change, TOS accept). Secretaire should surface "integration needs reconnection" on the attention radar when a refresh fails — tracked as future work.
