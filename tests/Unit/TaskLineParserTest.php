...
it('extracts mm-dd and coerces to current year at 09:00', function () {
    CarbonImmutable::setTestNow('2026-04-23 10:00:00');
    $r = TaskLineParser::parseLine('Book dentist 06-15');
    expect($r['title'])->toBe('Book dentist');
    expect($r['due_at'])->toBe('2026-06-15 09:00:00');
});

it('rolls past mm-dd to next year', function () {
    CarbonImmutable::setTestNow('2026-04-23 10:00:00');
    $r = TaskLineParser::parseLine('Renew vehicle reg 03-01');
    expect($r['due_at'])->toBe('2027-03-01 09:00:00');
});

it('keeps today as today, not next year', function () {
    CarbonImmutable::setTestNow('2026-04-23 10:00:00');
    $r = TaskLineParser::parseLine('Ship release 04-23');
    expect($r['due_at'])->toBe('2026-04-23 09:00:00');
});

it('ignores invalid mm-dd (Feb 30)', function () {
    CarbonImmutable::setTestNow('2026-04-23 10:00:00');
    $r = TaskLineParser::parseLine('Note 02-30 edge case');
    expect($r['due_at'])->toBeNull();
    expect($r['title'])->toBe('Note 02-30 edge case');
});

it('handles all token types on one line', function () {
    CarbonImmutable::setTestNow('2026-04-23 10:00:00');
    $r = TaskLineParser::parseLine('Call @alice about taxes #admin P2 05-03');
