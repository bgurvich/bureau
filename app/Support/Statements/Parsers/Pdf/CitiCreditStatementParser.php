...
/**
 * Citi credit-card statement PDF.
 *
 * Layout cues: "Citi" + "Credit Card Statement". "Transactions" section
 * lines: "MM/DD  MM/DD  description  amount" (trans date + post date).
 * Credits render with "CR" suffix or explicit negative sign.
 */
final class CitiCreditStatementParser implements StatementParser
{
    ...
    public function fingerprint(string|array $content): bool
    {
        ...
        return (bool) preg_match('/Citi(?:bank)?/i', $content)
            && preg_match('/Credit\s*Card\s*Statement|Card\s*Account/i', $content);
    }
    ...
    public function parse(string|array $content): ParsedStatement
    {
        ...
        return new ParsedStatement(
            bankSlug: 'citi_credit',
            bankLabel: 'Citi — Credit Card',
            accountLast4: $last4,
            periodStart: $start,
            periodEnd: $end,
    ...
    private function extractTransactions(string $text, int $assumeYear): array
    {
        ...
        foreach ($lines as $line) {
            // Citi credit lines: "MM/DD  MM/DD  description   amount"
