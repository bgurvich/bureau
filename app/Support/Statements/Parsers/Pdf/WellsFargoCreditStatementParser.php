...
/**
 * Wells Fargo credit-card statement PDF.
 *
 * Layout cues:
 *   "Wells Fargo" + "Credit Card Statement" or "Account ending in XXXX" +
 *   "Statement Period" near the top. Transactions have a "Transactions"
 *   section with rows: "MM/DD  description  amount". Credits show as
 *   negative values or are flagged with "CR"/"-" suffix.
 */
final class WellsFargoCreditStatementParser implements StatementParser
{
    ...
    public function fingerprint(string|array $content): bool
    {
        ...
        return preg_match('/Wells\s*Fargo/i', $content)
            && preg_match('/Credit\s*Card\s*Statement|Credit\s*Account/i', $content);
    }
    ...
    public function parse(string|array $content): ParsedStatement
    {
        ...
        $last4 = $this->matchFirst('/Account\s*ending\s*in\s*(\d{4})/i', $text)
            ?? $this->matchFirst('/Account\s*number[^0-9]*(\d{4})(?!\d)/i', $text);
        ...
        return new ParsedStatement(
            bankSlug: 'wellsfargo_credit',
            bankLabel: 'Wells Fargo — Credit Card',
            accountLast4: $last4,
            periodStart: $start,
            periodEnd: $end,
