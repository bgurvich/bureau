...
/**
 * Amex bank checking/savings statement (Rewards Checking / HYSA). Layout
 * cues: "American Express National Bank" / "High Yield Savings" header,
 * "Statement Period". Transaction rows: "MM/DD description amount".
 */
final class AmexCheckingStatementParser implements StatementParser
{
    ...
    public function fingerprint(string|array $content): bool
    {
        ...
        return (bool) preg_match('/American\s*Express.*National\s*Bank|High\s*Yield\s*Savings|Rewards\s*Checking/i', $content);
    }
    ...
    public function parse(string|array $content): ParsedStatement
    {
        ...
        $opening = $this->matchMoney('/(?:Opening|Beginning)\s*Balance[^\n]*?\$?\s*([\-0-9,\.]+)/i', $text);
        $closing = $this->matchMoney('/(?:Closing|Ending)\s*Balance[^\n]*?\$?\s*([\-0-9,\.]+)/i', $text);

        return new ParsedStatement(
            bankSlug: 'amex_checking',
            bankLabel: 'Amex — Checking/Savings',
            accountLast4: $last4,
            periodStart: $start,
            periodEnd: $end,
    ...
    private function extractTransactions(string $text, int $assumeYear): array
    {
        ...
        foreach ($lines as $line) {
            if (preg_match('/Withdrawals|Debits/i', $line)) {
                $withdrawalContext = true;

                continue;
            }
            if (preg_match('/Deposits|Credits|Interest/i', $line)) {
                $withdrawalContext = false;
